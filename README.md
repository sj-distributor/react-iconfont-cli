...
组件支持转发 `ref`：传入 `onClick` 时，`ref` 指向外层 `HTMLButtonElement`；没有 `onClick` 时，`ref` 指向内部 `SVGSVGElement`。按需导出的单图标组件也会将 `ref` 转发到 SVG。这使组件可以直接作为 antd v6 `Popover`、`Tooltip`、`Dropdown` 等浮层组件的锚点。
