...
import React, { CSSProperties, SVGAttributes, forwardRef } from 'react';
...
const IconAlipay = forwardRef<SVGSVGElement, Props>(({
  size = 16,
  color,
  style: _style,
  role = 'img',
  ...
  const accessibilityLabel = ariaLabel && ariaLabel.trim() ? ariaLabel.trim() : "alipay";

  return (
    <svg ref={ref} viewBox="0 0 1024 1024" width={size + 'rem'} height={size + 'rem'} style={style} role={role} aria-label={accessibilityLabel} focusable="false" {...rest}>
      <path
      ...
      <path
        ...
        fill={getIconColor(color, 1, '#5B8BD4', '#5B8BD4')}
      />
    </svg>
  );
});
